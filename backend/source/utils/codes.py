...
class CodeUtil:
    ...
    async def _create_code(self, code:str, event_id: int, session: AsyncSession) -> None:
    ...
    async def create_qr_code(self, code: str, session: AsyncSession) -> io.BytesIO:
    ...
    async def create_codes(self, event_id: int, session: AsyncSession) -> str:
        code: str = self.generate_random_code()
        await self._create_code(code, event_id, session)
