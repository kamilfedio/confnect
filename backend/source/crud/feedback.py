...
async def get_all(event_id: int, pagination: tuple[int, int], session: AsyncSession) -> list[Base]:
...
async def get_all(user_id: int, pagination: tuple[int, int], session: AsyncSession) -> list[Base]:
