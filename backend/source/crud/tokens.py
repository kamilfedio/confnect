...
async def get_by_id(id: str, token_type: TokenType, session: AsyncSession) -> Base | None:
    query = select(Token).where(Token.token == id, Token.type == token_type, Token.expiration_date > datetime.now())
    res = await session.execute(query)
    return res.scalars().one_or_none()    

async def get_user_tokens_by_id(user_id: int, token_type: TokenType, session: AsyncSession) -> list[Base]:
