...
async def register(user_create: UserCreate, session: AsyncSession = Depends(get_async_session)) -> dict[str, str]:
    ...
    access_token: str = await create_token(user.id, type=TokenType.ACCESS, session=session)
    refresh_token: str = await create_token(user.id, type=TokenType.REFRESH, session=session)
...
async def create_token(
    ...
    user: UserRead | bool = await authenticate_user(form_data.username, form_data.password, session)
...
async def refresh_token(refresh_token: str, session: AsyncSession = Depends(get_async_session)) -> dict[str, str]:
    ...
    token: Token | None = await get_token(refresh_token, session, token_type=TokenType.REFRESH)
    if not token:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Invalid refresh token')
    if token.expirated:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Token has been expirated')

    access_token: str = await create_token(token.user_id, type=TokenType.ACCESS, session=session)
